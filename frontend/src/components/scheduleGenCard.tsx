...
function ScheduleGenCard({ schedule, onSave, onDiscard, onPreview }: P) {
	...
	return (
		<div className='bg-white rounded-2xl p-5 mb-10 text-sm boxshadow'>
			...
			<div className="flex flex-row flex-wrap justify-between mb-2">
				<button 
					className='w-full sm:w-5/12 py-2 bg-gray-200 hover:bg-gray-300 rounded-xl mb-3'
					onClick={() => setHide(!hide)}
				>
					<FontAwesomeIcon icon={hide ? faCaretDown : faCaretUp} /> {hide ? 'Show' : 'Hide'} Courses
				</button>
				<button 
					className='w-full sm:w-5/12 py-2 bg-gray-200 hover:bg-gray-300 rounded-xl mb-3'
					onClick={() => onPreview()}
			...
			<div className='flex flex-row flex-wrap justify-between'>
				<button 
					className='py-2 mb-3 bg-gray-200 hover:bg-gray-300 rounded-xl w-full sm:w-5/12'
					onClick={() => {
						if (confirm) {
				...
				<button 
					className='py-2 mb-3 bg-slate-500 hover:bg-slate-400 rounded-xl text-white w-full sm:w-5/12'
					onClick={() => onSave()}
				>
